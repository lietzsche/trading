...
export function AIReport({report}) {
  return <article className="card ai-report"><h3>AI 분석 의견</h3><p className="ai-prose">{typeof report === 'string' ? report : '분석 설명이 없습니다.'}</p></article>;
}
...
export default function AIAnalysis({user, refreshToken = 0, setError}) {
  ...
  return <div className="ai-page">
    <section className="ai-intro"><div><span className="eyebrow">AI RESEARCH ASSISTANT</span><h2>설정을 바꾸기 전에,<br/>근거부터 비교하세요.</h2><p>현재 전략을 읽고 필요한 시세를 조회해 설정 후보를 분석합니다. AI는 주문하지 않으며, 설정도 승인 없이 바꾸지 않습니다.</p></div><span className="ai-mode">조회·분석 전용</span></section>
    ...
    <section className="card"><div className="section-head ai-section-head"><h3>새 분석</h3><span className="ai-muted">설정 변경과 주문은 실행하지 않습니다</span></div><form className="form" onSubmit={startAnalysis}>
      ...
      <button className="primary" disabled={!config?.configured || Boolean(pending) || running || atLimit || !prompt.trim()}>{pending === 'analysis' ? '분석 요청 중…' : running ? '진행 중인 분석을 기다려 주세요' : !config?.configured ? '먼저 DeepSeek 키를 등록해 주세요' : '자료 조회 및 AI 분석 시작'}</button>
    </form></section>

    <section className="ai-history"><div className="section-head ai-section-head"><h3>내 분석 기록 <small>{count(history.total)}건</small></h3>{loading && <span className="ai-muted" role="status">불러오는 중…</span>}</div>{!history.items?.length ? <div className="empty"><b>아직 분석 기록이 없습니다</b><span>분석을 실행하면 결과와 사용량이 저장됩니다.</span></div> : <div className="ai-history-list">{history.items.map(item => <button key={item.id} className={`ai-history-item ${String(item.id) === String(selectedId) ? 'selected' : ''}`} onClick={() => selectAnalysis(item.id)} aria-pressed={String(item.id) === String(selectedId)}><span><b>{marketName(item.market)} 분석</b><time>{timestamp(item.created_at)}</time></span><span><b className={`ai-status ${String(item.status).toLowerCase()}`}>{AI_STATUS_LABELS[item.status] || item.status}</b><small>{count(item.usage_tokens)} 토큰</small></span></button>)}</div>}{pages > 1 && <nav className="pager" aria-label="AI 분석 기록 페이지"><button className="quiet" disabled={page === 0 || loading} onClick={() => setPage(value => value - 1)}>이전</button><span>{page + 1} / {pages}</span><button className="quiet" disabled={page + 1 >= pages || loading} onClick={() => setPage(value => value + 1)}>다음</button></nav>}</section>

    {selectedId !== null && <section className="ai-result" aria-label="선택한 분석 결과">
      <div className="section-head ai-section-head"><h3>분석 결과 {selected && <small>· {marketName(selected.market)}</small>}</h3><button className="quiet compact" onClick={() => {detailRequests.current.cancel(); setSelectedId(null); setSelected(null); setProposal(null);}}>결과 닫기</button></div>
      {detailLoading && <div className="loading-row" role="status"><div className="loader"/>분석 결과를 확인하고 있습니다.</div>}
      {selected && <><p className="ai-muted">{timestamp(selected.created_at)} · {AI_STATUS_LABELS[selected.status] || selected.status} · {count(selected.usage_tokens)} 토큰{selected.include_account ? ' · 계좌 요약 포함' : ' · 계좌 요약 제외'}</p><details className="ai-details"><summary>분석 요청 보기</summary><p className="ai-prose">{selected.prompt}</p></details>
